...
window.Win10 = {
	...
	_init: function() {
		...
		Win10.setContextMenu('#win10>.desktop', [
			...
			['<i class="fa fa-fw fa-refresh"></i> ' + Win10.lang('刷新','刷新'), function() {
				location.reload()
			}],
			'|', ['<i class="fa fa-fw fa-window-maximize"></i> ' + Win10.lang('进入全屏', '进入全屏'), function() {
				Win10.enableFullScreen()
			}],
			['<i class="fa fa-fw fa-window-restore"></i> ' + Win10.lang('退出全屏', '退出全屏'), function() {
				Win10.disableFullScreen()
			}],
			'|', ['<i class="fa fa-fw fa-info-circle"></i> ' + Win10.lang('关于', '关于我们'), function() {
				Win10.aboutUs()
			}],
		]);
		Win10.setContextMenu('#win10_btn_group_middle', [
			['<i class="fa fa-fw fa-window-maximize"></i> ' + Win10.lang('全部显示', '全部显示'), function() {
				Win10.showWins()
			}],
			['<i class="fa fa-fw fa-window-minimize"></i> ' + Win10.lang('全部隐藏', '全部隐藏'), function() {
				Win10.hideWins()
			}],
			['<i class="fa fa-fw fa-window-close"></i> ' + Win10.lang('全部关闭', '全部关闭'), function() {
				Win10.closeAll()
			}],
		]);
	...
	exit: function() {
		layer.confirm(Win10.lang('请确认退出系统', '请确认是否退出系统?'), {
			icon: 3,
			title: Win10.lang('提示', '提示')
		}, function(index) {
